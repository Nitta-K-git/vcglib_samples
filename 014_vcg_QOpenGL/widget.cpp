...
//				"   highp vec3 color = vec3(0.39, 1.0, 0.0);\n"
	...
	program_pick->addShaderFromSourceCode(QOpenGLShader::Fragment, "void main(void){gl_FragColor = vec4(1.0, 0.0, 1.0, 1.0);}");
...
//	f->glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, indecies->constData());
	f->glDrawArrays(GL_TRIANGLES, 0, vertices->size());
//	f->glDrawElements(GL_TRIANGLES, indecies->size(), GL_UNSIGNED_INT, nullptr);
//	f->glDrawElements(GL_POINTS, indecies->size(), GL_UNSIGNED_INT, nullptr); // draw only vertex points
//	f->glDrawElements(GL_LINE_STRIP, indecies->size(), GL_UNSIGNED_INT, nullptr); // draw triangle lines(GL_LINE_LOOP is same result)
	
//	f->glDrawElements(GL_TRIANGLES, indecies->size(), GL_UNSIGNED_INT, indecies->constData());
	pick();
	
//	float ff;
//	f->glGetFloatv(GL_PROJECTION_MATRIX, &ff);
...
//	qDebug("rect: %f, %f, %f, %f", start.X(), start.Y(), cur.X(), cur.Y());
	...
	f->glDrawArrays(GL_LINE_LOOP, 0, pick_rect.size());
}
...
void Widget::pick(){
	...
	int hit = vcg::GLPickTri<CMeshO>::PickVisibleFace(f, M, mid[0], mid[1], mesh, NewSelFace, wid[0], wid[1]);
//	qDebug("hit:%d", hit);
